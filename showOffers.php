...
session_start();
       ...
       if (empty($_SESSION['id_student'])) {
        header('Location: http://localhost/final/homePageS.php');
        die();
        }
...
<li  ><a href="homePageS.php">Homepage</a></li>
			...
			<li class="active"><a href="showOffers.php">Offers</a></li>
		...
		<div id="searchbox" class="pull-right">
					<form action="showOffers.php" method="GET" >
						<div class="form-group">
							<label class="sr-only" for="searchfield">Searchbox</label>
							<input type="text" class="form-control" id="searchfield" placeholder="Type keywords and press enter" Name="Search" value="" >
						</div>
					</form>
				</div>
...
$query="SELECT * from offers WHERE jobtitle='$Search' OR salary='$Search'  ";
...
while ($row=mysqli_fetch_array($result)) {
	...
	echo '		<img src="images/search.png" alt="logo" class="img-responsive" />';
				echo '</div>';
		echo '<div class="col-lg-4 col-md-5 col-sm-6 col-xs-12 job_title">';
		echo "<p><strong>{$job_title}</p>";
...
<h5>All Jobs</h5>
    ...
    $query = "SELECT * FROM responsible ";
                ...
                {  
                    ...
                    $_SESSION['id_responsible'] =$id_responsible ;
                

$id_responsible = $_SESSION['id_responsible'];

$query="SELECT * from offers where id_respon = '$id_responsible'   ";
$result=mysqli_query($conn,$query);
  $file = $_SESSION['file'];
...
echo "<p><img src= " . "uploads/". basename($_SESSION['file']) . " class='img-responsive'/> </p> ";

//echo '<img src="logo" alt="logo" class="img-responsive" />';
				echo '</div>';
