...
<li  ><a href="homePageS.php">Homepage</a></li>
			...
			<li class="active"><a href="sturequst.php">watchlist Offers</a></li>
			...
			<div class="container">
				<div class="row">
					<div class="col-sm-12 text-center">
						...
						<h4>my applications to Training</h4>
					</div>
				</div>
			</div>
		</section>
...
<?php  
$query22 = "SELECT * from candidates WHERE id_stu = '$id_student' ";
$result4=mysqli_query($conn,$query22);
while ($row=mysqli_fetch_array($result4)) {
	       $id_offer1=$row['id_off'];
	       $state=$row[3];			
		

// FETCHING DATA 
$q20="SELECT * from offers WHERE id_offer ='$id_offer1'  ";
...
while ($row=mysqli_fetch_array($result3)) {
 ...
 $id_res=$row['id_respon'];
	...
	echo "<a href='#'  class='featured applied'>";
			echo '<div class="row">';
			
	echo '<div class="col-lg-1 col-md-1 hidden-sm hidden-xs">';
	echo '		<img src="images/download (2).png" alt="logo" class="img-responsive" />';
				echo '</div>';
...
echo "<p class='job-posted'>{$date}<strong></i></strong></p>";
		echo"<p class='badge Internship'>{$state}</i></p>";
					echo '</div>';
		    echo '	</div>';
			echo '</a>';
            echo "<br>";
