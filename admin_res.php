...
<body >
<form action= "admin_res.php" method="post" enctype="multipart/form-data">
...
<li class="active"><a href="admin_res.php">Validate Responsible</a></li>
...
include('dbconfig.php');
    ...
    $q200="SELECT * from responsible WHERE state !='validated' ";
        $result30=mysqli_query($conn,$q200); 
       while ($row=mysqli_fetch_array($result30)) {
        $id_res=$row['id_responsible'];
        $_SESSION['id_responsible']=$id_res;
        $name=$row['name'];
        $email=$row['email'];
			...
			echo "	<td class='cell100 column2'>{$id_res}</td>";
				echo "	<td class='cell100 column2'>{$name}</td>";
...
<?php 
session_start();
$id_res=$_SESSION['id_responsible'];
	 ...
	 if(isset($_POST["Accept"]))
{
 ...
 $a ="UPDATE responsible SET state ='validated' WHERE id_responsible =  '$id_res' ";
 $result11=mysqli_query($conn,$a);
}

	elseif (isset($_POST["Reject"])) {

$C ="DELETE FROM responsible WHERE id_responsible = '$id_res' ";
